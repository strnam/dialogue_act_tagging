...
# Gotit Dataset
flags.DEFINE_string('gotit_input_dir', '',
                    'The Gotit dataset input directory.')
flags.DEFINE_string('gotit_unlabeled_input_file', '',
                    'The unlabeled gotit dataset input file. If set, '
                    'the input file is used to augment RT and vocab.')
...
def gotit_documents(dataset='train',
                      include_unlabeled=False,
                      include_validation=False):
  """Generates Documents for DBpedia dataset.
  ...
  """
  import pandas as pd
  if not FLAGS.gotit_input_dir:
    raise ValueError('Must provide FLAGS.gotit_input_dir')

  tf.logging.info('Generating Gotit documents...')
  data_path = os.path.join(FLAGS.gotit_input_dir, dataset + '.csv')
  df = pd.read_csv(data_path, header=None)
  for idx in df.index:
    content = df.loc[idx, 0]
    label = 0 # label is not ignore
    yield Document(
          content=content,
          is_validation=False,
          is_test=False,
          label=int(label),  # Labels should start from 0
          add_tokens=True)
    # if FLAGS.gotit_unlabeled_input_file and include_unlabeled:
    #   unlabeled_data_path = os.path.join(FLAGS.gotit_input_dir, FLAGS.gotit_unlabeled_input_file)
    #   with open(unlabeled_data_path) as rt_f:
    #     for content in rt_f:
    #       yield Document(
    #           content=content,
    #           is_validation=False,
